...
### Active Channels
The active channels section allows the user to enable or disable channels that the following four functions will work on. For example, enabling only the R channel will mean that it is the only channel to be affected when the user presses the Invert button. Different combinations of RGBA can be selected by holding the shift key while clicking on the channel buttons.
...
### Fill / Clear
Fill sets the value of the currently active channel(s) to 1, wheras Clear will set it to 0.
...
### Posterize
Posterize the value of the currently active channel(s).
